import type { DatabaseRecord, TableData } from "./database.ts";
import type {
  Operation,
  OperationCount,
...
> = EventName extends `${OperationName}-one` ? ObserveOne<Rec>
  : EventName extends `${OperationName}-all` ? ObserveAll<Rec>
  : ObserveMany<Rec>;
